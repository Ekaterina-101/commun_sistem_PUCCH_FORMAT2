...
void BLER(double Es_N0_min = -6.0, double Es_N0_max = 6.0,
          ...
          std::string output_file = "BLER_data.txt") {
  // CODE_WORDS = generate_codewords();
  // for (int i = 0; i < CODE_WORDS.size(); i++) {
  //     print_vector(CODE_WORDS[i]);
  // }
  ...
  for (std::size_t i_k = 0; i_k < 5; i_k++) {
    ...
    for (double Es_N0 = Es_N0_min; Es_N0 <= Es_N0_max; Es_N0 += Es_N0_step) {
      int errors_word = 0;
...
int app_main(int argc, char *argv[]) {
  ...
  if (mode != "coding" && mode != "decoding" && mode != "channel simulation") {
  ...
  if (mode == "coding") {
    if (!input_json.contains("pucch_f2_bits")) {
      std::cerr << "Error: Missing 'pucch_f2_bits' field for 'coding' mode"
                << std::endl;
      return 8;
    }
    ...
    std::vector<int> bits = json_to_vector_int(input_json["pucch_f2_bits"]);
    ...
    if (!input_json.contains("qpsk_symbols")) {
      std::cerr << "Error: Missing 'qpsk_symbols' field for 'decoding' mode"
                << std::endl;
      return 10;
    }
    ...
    if (!input_json.contains("iterations") ||
      ...
      std::cerr << "Error: Missing or invalid 'iterations' field for 'channel "
                   "simulation' mode"
                << std::endl;
      return 12;
    }
    ...
    if (iterations <= 0) {
      std::cerr << "Error: 'iterations' must be positive (got " << iterations
                << ")" << std::endl;
      return 13;
    }

    int error_trans = failed_trans(num_bits, SNR_FOR_SIMULATION, iterations);
    if (error_trans == NAN) {
      return -4;
    }
  ...
  if (COUNTER_ERRORS != 0) {
    std::cerr << "Error: in PUCCH_F2 simulation" << std::endl;
    return -2;
  }
